...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.role === "recruiter" && !form.companyId)
      return toast.error(
        "Company ID is required. Contact admin to register your company first.",
      );
    setLoading(true);
    try {
      const payload = {
            ...
            {form.role === "recruiter" && (
              <div>
                <Input
                  ...
                  placeholder="Ask admin for your company ID"
                />
                <p className="mt-1 text-xs text-gray-400">
                  Admin must create the company first and share the ID with you.
                </p>
              </div>
            )}
